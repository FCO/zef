...
package Zef::Commands {
    ...
    sub fetch(
        ...
        *@identities ($, *@)
        ...
        return abort "Failed to resolve any candidates. No reason to proceed" unless +@candidates;
        ...
        +@fetched && +@fetched == +@candidates && +@fail == 0 ?? 0 !! 1;
    }
        ...
        *@wants ($, *@)
        ...
        my (:@paths, :@uris, :@identities) := @wants.classify: -> $wanted {
            $wanted ~~ /^[\. | \/]/                                           ?? <paths>
                ...
                !! return abort("Don't understand identity: {$wanted}");
        }
        ...
        # LOCAL PATHS
        return abort "The following were recognized as file paths but don't exist as such - {@paths.grep(!*.IO.e)}"
            if +@paths.grep(!*.IO.e);
        ...
        return abort "No candidates found matching uri: {@uri-candidates-to-check.join(', ')}" if +@uris && +@uri-candidates-to-check == 0;
        ...
        return abort "No candidates found matching identity: {@requested-identities.join(', ')}"\
        ...
        unless +@candidates {
            ...
            exit(0) if $deps-only;
            return abort("No reason to proceed. Use --force-install to continue anyway", 0) unless $force-install;
        }
        ...
        +@installed && +@installed == +@candidates && +@fail == 0 ?? 0 !! 1;
    }
    ...
    sub search(Int :$wrap = False, :$update, *@terms ($, *@)) is export {
    ...
    sub list(Int :$max?, :$update, Bool :i(:$installed), *@at) is export {
        my $client = get-client(:config($CONFIG), :$update);
    ...
    sub upgrade(
        ...
        *@identities
        ...
        return abort "Can't upgrade identities that aren't installed: {@missing.join(', ')}" if +@missing;
        ...
        return abort("All requested distributions are already at their latest versions", 0) unless +@upgradable;
        say "The following distributions will be upgraded: {@upgradable.map(*.dist.identity).join(', ')}";

        my &installer = &install.assuming(
            :$depends,
            :$test-depends,
            :$build-depends,
            :$test,
        ...
        my (:@upgraded, :@failed) := @sorted-candidates.map(*.uri).classify: -> $uri {
            ...
            try { &installer($uri) } ?? <upgraded> !! <failed>;
        }
        return abort "!!!> Failed upgrading *all* modules" unless +@upgraded;

        say "!!!> Some modules failed to update: {@failed.map(*.dist.identity).join(', ')}" if +@failed;
        +@upgraded < +@upgradable ?? 1 !! 0;
    }
    ...
    sub locate($identity, Bool :$sha1) is export {
        my $client = get-client(:config($CONFIG));
        if !$sha1 {
    ...
    sub browse($identity, $url-type where * ~~ any(<homepage bugtracker source>), Bool :$open = True) is export {
        my $client = get-client(:config($CONFIG));
    ...
    sub look($identity) is export {
        ...
        my $fetched = @local[0] || $client.fetch(@remote[0])[0] || return abort "Failed to fetch candidate: $identity";
        my $dist-path = $fetched.dist.path;
        say "===> Shelling into directory: {$dist-path}";
        so shell(%*ENV<SHELL> // %*ENV<ComSpec> // %*ENV<COMSPEC>, :cwd($dist-path)) ?? 0 !! 1;
    }
    ...
    sub update(*@names) is export {
        my $client  = get-client(:config($CONFIG));
        my %results = $client.recommendation-manager.update(@names);
        my $rows    = %results.map: {[.key, .value]};
        return abort "A plugin name was provided that does not exist or does not support 'update'"
            if +@names && (+@names > +$rows);
    ...
    sub nuke(Bool :$confirm, *@names ($, *@)) is export {
    ...
    multi sub abort(Str $str, Int $exit-code = 255) { say $str; $exit-code }
